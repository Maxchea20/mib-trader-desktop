...
def _hunt_thesis(hunt: Dict, extra: Optional[Dict] = None) -> Dict:
    """Freeze the 15m parent on FIRE so SI does not use a later 15m close."""
    th = {
...
def _in_cooldown(tf_seconds: int) -> bool:
    if STATE["last_close_ts"] is None:
        return False
    # SI / structure-cancel is not a money stop — do not lock the gun.
    fail = ("SL", "BRAIN_EXIT", "FLY", "cancel", "HARD_SL")
    bars = CONFIG["cooldown_bars_after_failure"] if STATE["last_close_reason"] in fail else CONFIG["cooldown_bars_normal"]
