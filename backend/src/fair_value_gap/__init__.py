...
def analyze(candles, timeframe: str) -> AgentResult:
    ...
    for i in range(2, n):
        ...
        gap_quality = clamp(gap_atr * 35.0, 0.0, 40.0)
        displacement_quality = clamp(displacement_atr * 25.0, 0.0, 30.0)
        recency_quality = clamp(30.0 - ((n - 1 - i) * 0.5), 0.0, 30.0)
        state_bonus = {"fresh": 20.0, "partial": 8.0, "filled": 0.0}[fvg["state"]]
        fvg["quality"] = clamp(gap_quality + displacement_quality + recency_quality + state_bonus, 0.0, 100.0)
        fvgs.append(fvg)

    if not fvgs:
        return neutral(AGENT_ID, timeframe, "No significant FVGs", valid=True)
    ...
    if not active:
        evidence = [f"{len(fvgs)} significant FVGs", "All detected FVGs fully mitigated"]
        key_levels = []
        for f in fvgs[-4:]:
            ...
            key_levels.append({
                ...
                "mitigated": True,
                "state": "filled",
                "quality": round(f["quality"], 1),
            })
        return AgentResult(AGENT_ID, NEUTRAL, 20.0, 10.0, evidence, key_levels, timeframe, valid=True)

    nearest = min(active, key=lambda f: abs(((f["lo"] + f["hi"]) / 2) - current_price))
    mid = (nearest["lo"] + nearest["hi"]) / 2
    distance_atr = abs(mid - current_price) / atr
    direction = LONG if nearest["dir"] == "bull" else SHORT
    confidence = 40.0 + nearest["quality"] * 0.35 - distance_atr * 8.0
    if nearest["state"] == "fresh":
        confidence += 8.0
    elif nearest["state"] == "partial":
        confidence -= 5.0
    confidence = clamp(confidence, 25.0, 90.0)
    strength = nearest["quality"] * 0.70 + max(0.0, 25.0 - distance_atr * 10.0)
    strength = clamp(strength, 10.0, 90.0)
    evidence = [f"{len(fvgs)} significant FVGs ({len(fresh)} fresh, {len(partial)} partial)"]
    if nearest["dir"] == "bull":
        evidence.append(f"Nearest bullish FVG {nearest['lo']:.1f}–{nearest['hi']:.1f} (support/demand)")
    else:
        evidence.append(f"Nearest bearish FVG {nearest['lo']:.1f}–{nearest['hi']:.1f} (resistance/supply)")
    evidence.append(f"FVG size={nearest['gap_atr']:.2f} ATR")
    evidence.append(f"Displacement={nearest['displacement_atr']:.2f} ATR")
    evidence.append(f"State={nearest['state']}")
    key_levels = []
    ranked = sorted(active, key=lambda f: (abs((f["lo"] + f["hi"]) / 2 - current_price), -f["quality"]))
    for f in ranked[:6]:
        timestamp = int(candles[f["i"]]["ts"])
        key_levels.append({
            "label": "FVG bull" if f["dir"] == "bull" else "FVG bear",
    ...
    return AgentResult(AGENT_ID, direction, round(confidence, 1), round(strength, 1), evidence, key_levels, timeframe, valid=True)


from .observe import observe  # Step 6D — AnalysisObservation producer
