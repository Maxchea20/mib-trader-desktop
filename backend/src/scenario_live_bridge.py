...
def evaluate_scenario(live_price: Optional[float]) -> Dict:
    ...
    if thesis_dbg is None:
        return {"action": "WAIT", "reason": "engine FIRE with no thesis debug -- refusing"}
    ...
    if any(w["thesis_id"] == thesis_id for w in _pending_watches.values()):
        return _skip(attempt_id, "an M1 C watch for this thesis is already running", log_base)
    if setup == "S1":
        enabled_slots = CONFIG.get("enabled_m5_slots", [1, 2])
        if slot is None:
            return _skip(attempt_id, "S1 M5 confirm outside the slots of its own 15m", log_base)
...
def _drive_pending_watches(out: Dict) -> Dict:
    ...
    if waiting is not None:
        return waiting
    return {
        "action": "WAIT", "reason": out.get("what_happening"), "scenario": out.get("scenario"),
...
def watcher_status() -> Dict:
    return {
        "active_c_watch_theses": _WATCHER.active_count(),
