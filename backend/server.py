...
@api_router.get("/")
async def root():
    return {"app": "MIB-Trader", "version": CONFIG_VERSION, "symbol": SYMBOL, "timeframes": TIMEFRAMES}

@api_router.get("/config")
async def get_config():
    return {"config_version": CONFIG_VERSION, "symbol": SYMBOL, "timeframes": TIMEFRAMES,
            "htf_timeframes": HTF_TIMEFRAMES, "ltf_timeframes": LTF_TIMEFRAMES,
            "agent_weights": AGENT_WEIGHTS, "htf_gate": HTF_GATE, "confluence": CONFLUENCE,
            "conflict": CONFLICT, "entry": ENTRY, "assumptions": ASSUMPTIONS}
...
async def market_candles(timeframe: str = Query("15m"), limit: int = Query(500)):
    ...
    return {"symbol": SYMBOL, "timeframe": timeframe, "count": len(candles), "candles": candles, "live": live}
...
async def backtest(timeframe: str = Query("15m"), lookback: int = Query(150), forward: int = Query(8), step: int = Query(3)):
    if timeframe not in TIMEFRAMES:
        return {"error": "invalid_timeframe", "timeframes": TIMEFRAMES}
    result = await asyncio.to_thread(backtest_engine.run_backtest, timeframe, lookback, forward, step)
...
async def walkforward_run(req: WalkForwardRunReq):
    ...
    if weights is not None:
        weights = {k: max(runtime_settings.WEIGHT_MIN, min(runtime_settings.WEIGHT_MAX, float(v))) for k, v in weights.items()}
    ...
    params = dict(timeframe=req.timeframe, days=req.days, start_balance=req.start_balance,
        capital_pct=req.capital_pct, leverage=req.leverage, sl_atr_mult=req.sl_atr_mult,
        tp_atr_mult=req.tp_atr_mult, weights_override=weights, htf_timeframes=req.htf_timeframes,
        ...
        pivot_window_override=req.pivot_window_override, use_weather=req.use_weather,
...
async def paper_open(req: OpenTradeReq):
    ...
    return paper_trading.open_trade(symbol=SYMBOL, side=req.side, entry_price=entry, sl_price=req.sl_price,
        tp_price=req.tp_price, notional_usd=req.notional_usd, timeframe=req.timeframe,
        brain_state=req.brain_state, consensus=req.consensus, confidence=req.confidence, note=req.note)
...
async def paper_trades(status: Optional[str] = Query(None)):
    live = manager.live_status()
    return {"trades": paper_trading.list_trades(status, live.get("last_price")), "live_price": live.get("last_price")}
...
async def mexc_account():
    ...
    try:
        ...
        return {"connected": False, "error": str(e), "live_armed": live_armed, "keys_present": keys_present,
                "hint": "Add MEXC_API_KEY, MEXC_API_SECRET, MEXC_LIVE_TRADING_ENABLED=true to .env and restart."}
    except Exception as e:
        return {"connected": False, "error": f"unexpected error: {e}", "live_armed": live_armed, "keys_present": keys_present}
    usdt = next((a for a in assets if a.get("currency") == "USDT"), None)
    if not usdt:
        return {"connected": True, "error": "No USDT asset entry in account", "live_armed": live_armed, "keys_present": keys_present}
    return {"connected": True, "currency": "USDT", "equity": float(usdt.get("equity") or 0),
            ...
            "live_armed": live_armed, "keys_present": keys_present}

@api_router.get("/mexc/probe")
async def mexc_probe():
    from src import mexc_ping
    return await asyncio.to_thread(mexc_ping.probe)

@api_router.post("/mexc/ping")
async def mexc_ping_order():
    from src import mexc_ping
    return await asyncio.to_thread(mexc_ping.ping)
...
async def decision_detail(decision_id: str):
    with decision_log.db._conn() as conn:
        ...
        agents = conn.execute("SELECT * FROM agent_decisions WHERE decision_id=?", (decision_id,)).fetchall()
...
async def setup_detail(setup_id: str):
    with decision_log.db._conn() as conn:
        ...
        checkpoints = conn.execute("SELECT * FROM market_checkpoints WHERE setup_id=? ORDER BY timestamp", (setup_id,)).fetchall()
        changes = conn.execute("SELECT * FROM market_changes WHERE setup_id=? ORDER BY timestamp", (setup_id,)).fetchall()
        decisions = conn.execute("SELECT * FROM decisions WHERE setup_id=? ORDER BY timestamp", (setup_id,)).fetchall()
    return {"setup": dict(setup), "checkpoints": [dict(c) for c in checkpoints],
            "changes": [dict(c) for c in changes], "decisions": [dict(d) for d in decisions]}
...
async def startup():
    ...
    logger.info("MEXC keys=%s live_armed=%s",
