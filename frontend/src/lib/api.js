...
export const getBacktestLog = async (timeframe) =>
  (await client.get("/backtest/log", { params: timeframe ? { timeframe } : {} })).data;
export const deleteBacktestLogEntry = async (id) =>
  (await client.delete(`/backtest/log/${id}`)).data;
export const clearBacktestLog = async (timeframe) =>
  (await client.delete("/backtest/log", { params: timeframe ? { timeframe } : {} })).data;

export const runWalkForwardBacktest = async (payload) =>
  (await client.post("/backtest/walkforward/run", payload)).data;
export const getWalkForwardStatus = async (id) =>
  (await client.get(`/backtest/walkforward/status/${id}`)).data;
export const getWalkForwardResult = async (id) =>
...
export const getPaperBalance = async () => (await client.get("/paper/balance")).data;
