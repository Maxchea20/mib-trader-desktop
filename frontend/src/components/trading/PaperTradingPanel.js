...
const LiveTradingControls = ({ auto, onSave }) => {
  ...
  return (
    <div className="panel mb-3 p-3" data-testid="live-trading-controls">
      ...
      {!armed && (
        <div className="mb-3 px-2 py-1.5 border border-amber-500/40 bg-amber-500/5 font-mono-t text-[11px] text-amber-300">Hunt is watching. No real order until backend/.env has MEXC_LIVE_TRADING_ENABLED=true and you restart.</div>
      )}
      {acct && !connected && (
      ...
      {sizing.error && (
        <div className="mb-3 px-2 py-1.5 border border-rose-500/40 bg-rose-500/5 font-mono-t text-[11px] text-rose-300" data-testid="sizing-error">Sizing not currently valid: {sizing.error}</div>
      )}
