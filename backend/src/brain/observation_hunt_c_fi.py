...
WAIT keeps living thesis side + thesis_level.
"""
...
def _origin_from_thesis(candles_15m: List[dict], pivot: int) -> Optional[float]:
    if not candles_15m or len(candles_15m) < 60:
        return None
    try:
        st = obs_structure(candles_15m, "15m", pivot_window_override=pivot)
    ...
    if last is None:
        return None
    raw = last.reference_price if getattr(last, "reference_price", None) else last.price
    try:
        v = float(raw or 0)
...
def _attach_thesis(fire, *, side, event, gate, ev_ts, candles_15m, pivot, rearm):
    ...
    try:
        level = float(hunt.get("level") or fire.get("entry") or fire.get("thesis_level") or 0) or None
    except (TypeError, ValueError):
        level = None
    if level is None:
        level = _origin_from_thesis(candles_15m, pivot)
    ...
    if side in (LONG, SHORT):
        fire["direction"] = side
    pack = dict(hunt)
    if level and not pack.get("level"):
        pack["level"] = level
    fire["hunt"] = pack
    return fire


def _wait_alive(why, *, side, event, gate, ev_ts, candles_15m, pivot, rearm):
    out = _wait(why)
    out["action"] = "WAIT"
    out["gate"] = gate
    return _attach_thesis(
        out, side=side, event=event, gate=gate, ev_ts=ev_ts,
        candles_15m=candles_15m, pivot=pivot, rearm=rearm,
    )
...
def _evaluate_hunt_c_fi_core(
    ...
    if setup:
        side, event = setup
        got = active_setup(candles_15m, pivot=2, require_fresh=True)
        ev_ts = got[2] if got else None
        if wx and not side_allowed(wx.get("flag"), side):
            way = "long" if side == LONG else "short"
            return done(_wait_alive(
                f"4-hour weather is {wx.get('flag')}, so no {way} trade now.",
                side=side, event=event, gate="internal", ev_ts=ev_ts,
                candles_15m=candles_15m, pivot=2, rearm=False,
            ), True, event)
        ...
        if fire:
            ...
            fire = _attach_thesis(
                ...
                ev_ts=ev_ts, candles_15m=candles_15m, pivot=2, rearm=False,
            )
    ...
    if wx and not side_allowed(wx.get("flag"), side):
        way = "long" if side == LONG else "short"
        return done(_wait_alive(
            f"4-hour weather is {wx.get('flag')}, so no {way} re-entry now.",
            side=side, event=event, gate=gate, ev_ts=ev_ts,
            candles_15m=candles_15m, pivot=pivot, rearm=True,
        ), True, event)
    fire = _try_answer(side, event, live, candles_15m, candle_5m, candles_4h, candles_5m, slot)
    if fire:
        fire["brain_version"] = HUNT_VERSION_C_FI
        ...
        return done(fire, True, event)
    return done(_wait_alive(
        "15m thesis is still valid. Waiting for the 5-minute candle to answer again.",
        side=side, event=event, gate=gate, ev_ts=ev_ts,
        candles_15m=candles_15m, pivot=pivot, rearm=True,
    ), True, event)
