...
for _ in range(3):  # simulate a few 5-second polling ticks
    out = watcher.check(sample["thesis_id"], direction, level, origin_ts)
    if out is not None:
        result = out
        break
...
first = watcher2.check(sample["thesis_id"], direction, level, origin_ts)
second = watcher2.check(sample["thesis_id"], direction, level, origin_ts)
...
missing_result = watcher3.check("FAKE-THESIS-MISSING", "LONG", 100.0, m5_2_open_ts)
